...
import { AuthResponseData, AuthService } from '../auth/auth.service'; //used to trigger the onLogin method
...
export class RegisterPage implements OnInit {
  ...
  constructor(private alertCtrl: AlertController, private authService: AuthService, private router: Router, private loadingCtrl: LoadingController, public formbuilder: FormBuilder, private menu: MenuController, private http: HttpClient) //Parameters injected to trigger the necessary methods
  {
    ...
    this.loadingCtrl.create({ keyboardClose: true, message: 'Creating account...' })
      .then(loadingEl => {
        ...
        this.http.post<AuthResponseData>(`https://identitytoolkit.googleapis.com/v1/accounts:signUp?key=${environment.firebaseAPIKey}`,
          { email: email, password: password, confirmpassword: confirmpassword, returnSecureToken: true }).subscribe(
